...
function back_prop(x, y, result) {
  ...
  //A is the layer after activation, Z is the layer before activation. Z2 = W2A1 + B2; A2 = A(Z2)
...
function make_prediction(verbose, use_training) {
  ...
  for (let i = 0; i < predictionData.length; i++) {
    ...
    if (!predictionData[i].input || predictionData[i].input.includes(undefined)) {
      ...
      if (verbose == 1) {
        console.log("Predicting " + prediction + " from A3: " + math.format(result.a3, { notation: 'fixed', precision: 4 }) + " vs actual ", correct);
      }
  ...
  console.log(correct_guesses + " / " + total + " predictions correct. " + (correct_guesses / total) + " accuracy value.")
}

function export_to_json(filename) {
  const jsonData = {};
  jsonData.weights1 = weights1.toArray();
  jsonData.biases1 = biases1.toArray();
  jsonData.weights2 = weights2.toArray();
  jsonData.biases2 = biases2.toArray();
  jsonData.weights3 = weights3.toArray();
  jsonData.biases3 = biases3.toArray();

  fs.writeFileSync(filename, JSON.stringify(jsonData, null, 2));
}

//Gets inputs as grayscale arrays, for use in testing against my web app model and to load the image onto a canvas from the values.
function export_inputs_to_json(filename) {
